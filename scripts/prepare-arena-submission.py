...
def main():
    ...
    (args.output / 'pr-body.md').write_text(
        'Add lean4cobol, a GnuCOBOL port of lean4lean’s executable kernel, pinned to '
        f'`{revision}`. The launcher has no wall-clock timeout.\n\n'
        'Validation and full Mathlib/reference timings are documented in the source '
        'repository’s `docs/mathlib-performance.md`. The checker uses exit codes '
        '0/1/2 for acceptance/rejection/resource decline.\n')
    print(args.output)
